...
yup.addMethod<yup.DateSchema>(yup.date, 'startTime', function (msg?: { lessThan?: Message<{}>, between?: Message<{}>}) {
  return this.test('isValidStartTime', msg, function (value) {
    ...
    const mileStone = (options?.from?.[1]?.value?.mileStone || []).filter((item: any, i: number) =>
      i !== index &&
    ...
    // const duplicate = mileStone.find(item => startTime.isSameOrAfter(moment(item.startTime)) || startTime.isSameOrBefore(moment(item.endTime)))
    // if (duplicate) {
    //   return createError({
    //     path,
    //     params: {
    //       lessThan: moment(duplicate.startTime).format('hh:mm'),
    //       greaterThan: moment(duplicate.endTime).format('hh:mm')
    //     },
...
yup.addMethod<yup.DateSchema>(yup.date, 'endTime', function (msg?: { moreThan?: Message<{}>, between?: Message<{}>}) {
  return this.test('isValidEndTime', msg, function (value) {
    ...
    // const duplicate = mileStone.find(item => endTime.isSameOrAfter(moment(item.startTime)) || endTime.isSameOrBefore(moment(item.endTime)))
    // if (duplicate) {
    //   return createError({
    //     path,
    //     params: {
